...
describe('6 test valid first name 64 characters', () => {
    before(() => {
        cy.visit('/');
    });

    it('Checking the First name field, enter a valid value of 64 characters.', () => {

        cy.get('.contact-us-header').scrollIntoView()
        cy.get('#mat-input-0').type(testData.validTestData.valid_first_name_64_characters)
                cy.log("Проверка поля'First name', вводим допустимое значение 64 символа.")
