...
describe('5 test valid first name 1 character', () => {
    before(() => {
        cy.visit('/');
    });

    it('Checking the First name field, enter a valid value of 1 character.', () => {

        cy.get('.contact-us-header').scrollIntoView()
        cy.get('#mat-input-0').type(testData.validTestData.valid_first_name_1_character)
                cy.log("Проверка поля'First name', вводим допустимое значение 1 символ.")
