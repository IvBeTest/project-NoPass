...
describe('8 test valid last name 64 characters', () => {
    before(() => {
        cy.visit('/');
    });

    it('Checking the Last name field, enter a valid value of 64 characters.', () => {

        cy.get('.contact-us-header').scrollIntoView()
        cy.get('#mat-input-0').type(testData.validTestData.first_name)
        cy.get('#mat-input-1').type(testData.validTestData.valid_last_name_64_characters)
